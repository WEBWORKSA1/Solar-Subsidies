/**
 * /api/admin.js — Admin dashboard backend
 * 
 ...
 *   GET  /api/admin?action=vendors&tier=&active=                  → admin_vendor_health
...
async function reassignLead(req, res) {
  ...
  // Find current assignment, mark as overridden
  ...
  if (current && current.length > 0 && current[0].vendor_id) {
    // Mark current as admin_reassigned
    ...
    // Exclude current vendor + any explicitly excluded
...
async function markInvoiced(req, res) {
  const { assignmentId, invoiceNumber } = req.body;
  ...
  await sbPatch(`lead_assignments?id=eq.${assignmentId}`, update);
  return res.status(200).json({ success: true });
}

async function markPaid(req, res) {
  const { assignmentId, paidDate } = req.body;
  if (!assignmentId) return res.status(400).json({ error: 'assignmentId required' });
  
  await sbPatch(`lead_assignments?id=eq.${assignmentId}`, {
